...
export function B2BToolRequestGatewayScreen() {
  ...
  const handleSubmit = () => {
    if (!bank || !gatewayType) {
      toast.error("لطفاً بانک و نوع درگاه را انتخاب کنید");
      return;
    }
    setSubmitted(true);
  };
  ...
  if (submitted) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] px-6 text-center">
        ...
        <p className="text-sm text-muted-foreground mb-2">شماره پیگیری: {fa("14050418003")}</p>
        <p className="text-xs text-muted-foreground mb-6">فعال‌سازی درگاه ظرف ۵ روز کاری</p>
  ...
  return (
    <div className="pb-24 px-4 pt-4 space-y-4">
      <div className="rounded-2xl bg-[#034ea2]/5 border border-[#034ea2]/20 p-4 flex items-start gap-3">
        <Globe className="w-5 h-5 text-[#034ea2] mt-0.5 shrink-0" />
        <p className="text-xs text-muted-foreground leading-relaxed">
          درخواست درگاه پرداخت آنلاین. بانک و نوع درگاه را انتخاب کنید.
        </p>
      </div>
      ...
      <div className="space-y-3">
        ...
        <div>
          <label className="text-xs font-bold text-muted-foreground mb-1.5 block">نوع درگاه *</label>
          <div className="grid grid-cols-2 gap-2">
            {["درگاه وب", "درگاه اپلیکیشن", "درگاه API", "درگاه تست"].map((t) => (
              <button key={t} onClick={() => setGatewayType(t)}
                className={cn("p-3 rounded-xl border-2 text-sm font-bold transition-all",
                  gatewayType === t ? "border-[#034ea2] bg-[#034ea2]/5" : "border-border bg-card")}>
                {t}
              </button>
            ))}
          </div>
        ...
        <div>
          <label className="text-xs font-bold text-muted-foreground mb-1.5 block">آدرس سایت</label>
          <input value={siteUrl} onChange={(e) => setSiteUrl(e.target.value)} placeholder="https://example.com"
            className="w-full h-12 px-4 rounded-xl bg-card border border-border text-sm focus:border-[#034ea2] outline-none" dir="ltr" />
        </div>
