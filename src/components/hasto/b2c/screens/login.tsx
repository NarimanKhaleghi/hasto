...
export function LoginScreen() {
  ...
  const handleOtpChange = (idx: number, value: string) => {
    ...
    if (newOtp.every((d) => d !== "") && newOtp.join("").length === 5) {
      // Mock: any code is valid (per spec)
      setTimeout(() => {
        setOtpSuccess(true);
        setTimeout(() => {
          setStep("success");
          setAuthenticated(true);
          setTimeout(() => setB2CScreen("dashboard"), 800);
        }, 600);
      }, 200);
    }
  };
  ...
  return (
    <div className="flex-1 flex flex-col bg-gradient-to-b from-[#e8f0fe] via-[#f8fafc] to-white dark:from-[#0a0f1a] dark:via-[#0a0f1a] dark:to-[#111827] h-full overflow-hidden">
      ...
      <div className="flex-1 flex flex-col items-center justify-center px-6 pt-12 pb-8">
        ...
        >
          <div className="w-24 h-24 rounded-3xl bg-gradient-to-br from-[#034ea2] to-[#023069] flex items-center justify-center shadow-2xl shadow-[#034ea2]/30 mb-6">
            <HastoLogo size={56} />
          </div>
          <h1 className="text-3xl font-bold mb-2 bg-gradient-to-l from-[#034ea2] to-[#0456B5] bg-clip-text text-transparent dark:from-white dark:to-[#9DBFF9]">
            هستو
          </h1>
        ...
        <motion.div
          ...
          className="flex flex-wrap gap-2 justify-center mt-8 max-w-[320px]"
        >
          {[
            { icon: ShieldCheck, label: "امن" },
            { icon: Sparkles, label: "هوشمند" },
            { icon: Check, label: "سریع" },
          ].map((f) => (
            <div
              key={f.label}
      ...
      <AnimatePresence mode="wait">
        {step === "phone" && (
          ...
          >
            <div className="text-center mb-5">
              <h2 className="font-bold text-lg mb-1">ورود به حساب</h2>
            ...
            <button
              ...
              disabled={!phoneValid}
              className={cn(
                "w-full h-14 rounded-2xl font-bold text-base mt-4 transition-all",
                phoneValid
            ...
            <p className="text-[11px] text-center text-muted-foreground mt-4 leading-relaxed">
              با ورود به هستو، شرایط استفاده و حریم خصوصی را می‌پذیرید.
            </p>
          </motion.div>
        )}
        ...
        {step === "otp" && (
          ...
          >
            ...
            <div className="mt-6 p-3 rounded-xl bg-[#034ea2]/5 dark:bg-[#034ea2]/10 border border-[#034ea2]/10 flex items-start gap-2">
              <Sparkles className="w-4 h-4 text-[#034ea2] dark:text-[#6BA0F5] shrink-0 mt-0.5" />
              <p className="text-[11px] text-muted-foreground leading-relaxed">
                در نسخه نمایشی، هر کد ۵ رقمی معتبر است. کافیست ۵ رقم وارد کنید.
              </p>
            </div>
          </motion.div>
        )}
        ...
        {step === "success" && (
          ...
          >
            ...
            <h2 className="text-xl font-bold mb-1">خوش آمدید!</h2>
            <p className="text-sm text-muted-foreground">در حال ورود به هستو...</p>
          </motion.div>
        )}
      </AnimatePresence>
    </div>
